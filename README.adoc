...
Hinter diesem Aufruf verbergen sich ein paar als default gesetzte Tasks:
...
`bake`::
jBake baut die Microsite zusammen
`htmlSanityCheck`::
die generierte Microsite wird auf Fehler überprüft
`copyReports`::
der daraus entstandene Test-Report wird in den Verzeichnisbaum der Microsite kopiert.
`bakePreview`::
